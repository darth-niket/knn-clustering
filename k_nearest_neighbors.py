...
import numpy as np
from utils import euclidean_distance, manhattan_distance, getNeighbours
...
class KNearestNeighbors:
    ...
    def predict(self, X):
        ...
        for i in range(X.shape[0]):
            neighbours = getNeighbours(X[i], self._X, self._y, self.n_neighbors, self._distance)
            output = []
            for row in range(len(neighbours)):
                output.append(neighbours[row][1])

            prediction = max(set(output), key=output.count)
            max_vals.append(prediction)
        return max_vals
